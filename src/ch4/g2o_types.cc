...
void EdgeInertial::linearizeOplus() {
    ...
    // double delta = 1e-6;
    // Vec3d v1_delta(delta, delta, delta);
    // Mat3d jacobian_estimate(Mat3d::Zero());
    
    // Vec3d ep_positive = RiT * (p2->estimate().translation() - p1->estimate().translation() - (v1->estimate()+v1_delta) * dt_ -
    //                         grav_ * dt_ * dt_ / 2) -
    //                      dp;

    // Vec3d ep_negative = RiT * (p2->estimate().translation() - p1->estimate().translation() - (v1->estimate()-v1_delta) * dt_ -
    //                         grav_ * dt_ * dt_ / 2) -
    //                      dp;

    // _jacobianOplus[1].block<3, 1>(6, 0) = (ep_positive - ep_negative) / (2 * delta);
    // _jacobianOplus[1].block<3, 1>(6, 1) = (ep_positive - ep_negative) / (2 * delta);
    // _jacobianOplus[1].block<3, 1>(6, 2) = (ep_positive - ep_negative) / (2 * delta);
    //TODO:END
