...
bool Fusion::SearchRTK() {
    ...
    std::for_each(std::execution::par_unseq, search_poses.begin(), search_poses.end(),
                  [this](GridSearchResult& gr) { AlignForGridNdt(gr); });
...
void Fusion::AlignForGridNdt(sad::Fusion::GridSearchResult& gr) {
    // pcl::NormalDistributionsTransform<PointType, PointType> ndt;
    // ndt.setTransformationEpsilon(0.05);
    // ndt.setStepSize(0.7);
    // ndt.setMaximumIterations(40);
    ...
    Ndt3d ndt3d_1;
    ndt3d_1.SetSource(current_scan_);
    std::vector<double> res{10.0, 5.0, 4.0, 3.0, 1.0};
    SE3 T = gr.pose_;
    for(auto r : res){
        ndt3d_1.grids_.clear();
        for(auto mp : map_data_){
            std::ifstream fin("../data/ch9/map_data/res_"+ std::to_string((int)r) + "/" + std::to_string(mp.first[0]) +
                                "_" + std::to_string(mp.first[1]) + ".txt");
            ...
            double info_1, info_2, info_3,info_4,info_5,info_6,info_7,info_8,info_9;
            while (!fin.eof()) {
                
                Eigen::Matrix<int, 3, 1> key_;
                
                fin >> k_0 >> k_1 >> k_2 >> mu_0 >> mu_1 >> mu_2 
                    >> info_1 >> info_2>> info_3>>info_4>>info_5>>info_6>>info_7>>info_8>>info_9;

                key_ << k_0, k_1, k_2;
                
                sad::Ndt3d::VoxelData voxel;
                voxel.mu_ << mu_0, mu_1, mu_2;
                
                voxel.info_ << info_1, info_2, info_3,info_4,info_5,info_6,info_7,info_8,info_9;
                if(ndt3d_1.grids_.find(key_) == ndt3d_1.grids_.end()){
                    ndt3d_1.grids_[key_] = voxel;
                }
                // LOG(INFO) << "key: " << key_.transpose();
                // LOG(INFO) << "mu: " << voxel.mu_.transpose();
                // LOG(INFO) << "添加一个网格完毕" << " " << i++;
            }
        }
        // LOG(INFO) << "全部网格添加成功";
    ...
    gr.score_ = ndt3d_1.ComputeScore(T);
    gr.result_pose_ = T;
    LOG(INFO) << "score: " << gr.score_;
}

bool Fusion::LidarLocalizationNdt() {
    SE3 pred = eskf_.GetNominalSE3();
    LoadNdtMap(pred);

    ndt3d_.SetSource(current_scan_);
    ndt3d_.AlignNdt(pred);
...
void Fusion::LoadNdtMap(const SE3& pose){
    ...
    if (map_data_changed) {
        ...
        ndt3d_.grids_.clear();
        for(auto mp : map_data_){
            std::ifstream fin("../data/ch9/map_data/res_1/"+ std::to_string(mp.first[0]) +
                                "_" + std::to_string(mp.first[1]) + ".txt");
            ...
            while (!fin.eof()) {
                ...
                voxel.info_ << info_1, info_2, info_3,info_4,info_5,info_6,info_7,info_8,info_9;
                if(ndt3d_.grids_.find(key_) == ndt3d_.grids_.end()){
                    ndt3d_.grids_[key_] = voxel;
                }
                // LOG(INFO) << "key: " << key_.transpose();
                // LOG(INFO) << "mu: " << voxel.mu_.transpose();
                // LOG(INFO) << "添加一个网格完毕" << " " << i++;
