...
double Ndt3d::ComputeScore(const SE3& pose) {
    ...
    std::vector<double> score(total_size);
    ...
    double gauss_d2_ = -2 * log ((-log ( gauss_c1 * exp ( -0.5 ) + gauss_c2 ) - gauss_d3) / gauss_d1_);
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](int idx) {
        ...
        for (int i = 0; i < nearby_grids_.size(); ++i) {
            ...
            if (it != grids_.end()) {
                ...
                double e_x_cov_x = exp(-gauss_d2_ * e.dot(v.info_ * e) / 2);
                double score_inc = -gauss_d1_ * e_x_cov_x;
                score[real_idx] = score_inc;
    ...
    score_ = total_score / source_->points.size();
    return score_;
}
...
bool Ndt3d::AlignNdt(SE3& init_pose) {
    ...
    for (int iter = 0; iter < options_.max_iteration_; ++iter) {
        ...
        for (int idx = 0; idx < effect_pts.size(); ++idx) {
            ...
            H += jacobians[idx].transpose() * 
                 (cauchy.rho_1 * Mat3d::Identity() + 2 * cauchy.rho_2 * errors[idx] * errors[idx].transpose()) * 
                 infos[idx] * jacobians[idx];
            err += -cauchy.rho_1 * jacobians[idx].transpose() * infos[idx] * errors[idx];
        }
