...
class Icp3d {
   public:
    ...
    {
        CauchyKernel(){}
        CauchyKernel(double e) : e_(e) {
            rho_ = c_ * c_ * log(1 + ((e_ * e_) / (c_ * c_)));
            rho_1 = 1 / (1 + ((e_ * e_) / (c_ * c_)));
            rho_2 = -1 / (c_ * c_) * rho_1 * rho_1;
        }
        CauchyKernel(Vec3d e):e_3dim_(e){
            rho_ = c_ * c_ * log(1 + ((e_3dim_.dot(e_3dim_)) / (c_ * c_)));
            rho_1 = 1 / (1 + ((e_3dim_.dot(e_3dim_)) / (c_ * c_)));
            rho_2 = -1 / (c_ * c_) * rho_1 * rho_1;
        }
    ...
    void ComputeResidualAndJacobians(const SE3& input_pose, Mat18d& HTVH, Vec18d& HTVr);
    void ComputeResidualAndJacobiansSimple(const SE3& input_pose, Mat18d& HTVH, Vec18d& HTVr);

    std::shared_ptr<KD_TREE<pcl::PointXYZI>> ikdtree_ = nullptr;
