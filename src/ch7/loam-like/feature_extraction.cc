...
void FeatureExtraction::Extract(FullCloudPtr pc_in, CloudPtr pc_out_edge, CloudPtr pc_out_surf, CloudPtr pc_out_ground) {
    ...
    for (int i = 0; i < num_scans; i++) {
        ...
        for (int j = 0; j < 6; j++) {
            ...
            ExtractFromSector(scans_in_each_line[i], sub_cloud_curvature, pc_out_edge, pc_out_surf, i, pc_out_ground);
        }
    }
}

void FeatureExtraction::ExtractFromSector(const CloudPtr &pc_in, std::vector<IdAndValue> &cloud_curvature,
                                          CloudPtr &pc_out_edge, CloudPtr &pc_out_surf, const int & num, CloudPtr& pc_out_ground) {
    // 按曲率排序
    ...
    //todo
    //选取地面上的点
    std::vector<int> picked_ground_points;
    if(num <= 6){
        double dis_trh = 1.35 / sin(15 - 2 * num);
        for (int i = 0; i <= (int)cloud_curvature.size() - 1; i++) {
            int ind = cloud_curvature[i].id_;
            if (std::find(picked_ground_points.begin(), picked_ground_points.end(), ind) == picked_ground_points.end()){
                if(sqrt(pow(pc_in->points[ind].x, 2) + pow(pc_in->points[ind].y, 2) + pow(pc_in->points[ind].z, 2)) >= dis_trh){
                    picked_ground_points.push_back(ind);
    ...
    for (int i = cloud_curvature.size() - 1; i >= 0; i--) {
        ...
        if (std::find(picked_points.begin(), picked_points.end(), ind) == picked_points.end()) {
            ...
            if (largest_picked_num <= 20) {
                ...
                //todo
                // if(num == 1){
                //     double now_distance = sqrt(pow(pc_in->points[ind].x, 2) + pow(pc_in->points[ind].y, 2) + pow(pc_in->points[ind].z, 2));
                //     max_diatance = now_distance >= max_diatance ? now_distance : max_diatance;
                // }
                //end
                point_info_count++;
            } else {
    ...
    for (int i = 0; i <= (int)cloud_curvature.size() - 1; i++) {
        int ind = cloud_curvature[i].id_;
        if (std::find(picked_points.begin(), picked_points.end(), ind) == picked_points.end() 
             && std::find(picked_ground_points.begin(), picked_ground_points.end(), ind) == picked_ground_points.end()) {
