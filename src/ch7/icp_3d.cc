        ...
        auto H_and_err = std::accumulate(
            ...
            [&jacobians, &errors, &effect_pts, &total_res, &effective_num](const std::pair<Mat6d, Vec6d>& pre,
                ...
                if (!effect_pts[idx]) {
                    ...
                    //TODO
                    CauchyKernel cauchy(errors[idx]);
                    total_res += errors[idx].dot(errors[idx]);
                    effective_num++;
                    return std::pair<Mat6d, Vec6d>(pre.first + jacobians[idx].transpose() * 
                                                        (cauchy.rho_1 * Mat3d::Identity() + 2 * cauchy.rho_2 * errors[idx] * errors[idx].transpose()) * 
                                                        jacobians[idx],
                                                   pre.second - cauchy.rho_1 * jacobians[idx].transpose() * errors[idx]);
                }
            });
...
bool Icp3d::AlignP2Plane(SE3& init_pose) {
    ...
    for (int iter = 0; iter < options_.max_iteration_; ++iter) {
        ...
        auto H_and_err = std::accumulate(
            index.begin(), index.end(), std::pair<Mat6d, Vec6d>(Mat6d::Zero(), Vec6d::Zero()),
            [&jacobians, &errors, &effect_pts, &total_res, &effective_num, this](const std::pair<Mat6d, Vec6d>& pre,
                                                                           int idx) -> std::pair<Mat6d, Vec6d> {
                if (!effect_pts[idx]) {
                    return pre;
                } else {
                    ...
                    effective_num++;
                    return std::pair<Mat6d, Vec6d>(pre.first + jacobians[idx].transpose() * 
                                                        (cauchy.rho_1 + 2 * cauchy.rho_2 * errors[idx] * errors[idx]) * 
                                                        jacobians[idx],
                                                   pre.second - cauchy.rho_1 * jacobians[idx].transpose() * errors[idx]);
                }
            });
...
void Icp3d::ComputeResidualAndJacobians(const SE3& input_pose, Mat18d& HTVH, Vec18d& HTVr){
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](int idx) {
        ...
        std::vector<PointType, Eigen::aligned_allocator<PointType>> nn;
        std::vector<float> dis(5);
        ikdtree_->Nearest_Search(ToPointType(qs), 5, nn, dis);  // 这里取5个最近邻
        if (nn.size() > 3) {
            // convert to eigen
            std::vector<Vec3d> nn_eigen;
            for (int i = 0; i < nn.size(); ++i) {
    ...
    for (int idx = 0; idx < effect_pts.size(); ++idx){
        ...
        total_res += errors[idx] * errors[idx];
        effective_num++;
        HTVH += jacobians[idx].transpose() * jacobians[idx] * 0.01;
        HTVr += -jacobians[idx].transpose() * errors[idx] * 0.01;
    }
    if (effective_num < options_.min_effective_pts_) {
        LOG(WARNING) << "effective num too small: " << effective_num;
        return;
    ...
    std::cout << "HTVH" << std::endl << HTVH << std::endl << "HTVr" << std::endl << HTVr.transpose() << std::endl;
}
...
void Icp3d::ComputeResidualAndJacobiansSimple(const SE3& input_pose, Mat18d& HTVH, Vec18d& HTVr){
    LOG(INFO) << "aligning with point to plane";
    assert(target_ != nullptr && source_ != nullptr);
    // 整体流程与p2p一致，读者请关注变化部分
    ...
    auto H_and_err = std::accumulate(
        ...
        [&jacobians, &errors, &effect_pts, &total_res, &effective_num, this](const std::pair<Mat18d, Vec18d>& pre,
