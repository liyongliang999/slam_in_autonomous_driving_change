...
class EdgeSE2PointXY : public g2o::BaseUnaryEdge<2, Vec2d, VertexSE2> {
   public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    EdgeSE2PointXY(Vec2d pt, float r, float angle, float theta):pt_(pt), r_(r), angle_(angle), theta_(theta) {}
    ...
    void linearizeOplus() override {
        _jacobianOplusXi << 1, 0,
                            0, 1,
                            -r_ * std::sin(angle_ + theta_), r_ * std::cos(angle_ + theta_);
...
class EdgeSE2PointLine : public g2o::BaseUnaryEdge<1, double, VertexSE2> {
   public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    EdgeSE2PointLine(Vec3d line_coeffs, float r, float angle, float theta):line_coeffs_(line_coeffs), r_(r), angle_(angle), theta_(theta) {}

    void computeError() override {
        VertexSE2* v1 = (VertexSE2*)_vertices[0];
        SE2 pose = v1->estimate();
        Vec2d pw = pose * Vec2d(r_ * std::cos(angle_), r_ * std::sin(angle_));
        _error[0] = line_coeffs_[0] * pw.x() + line_coeffs_[1] * pw.y() + line_coeffs_[2];
    }

    void linearizeOplus() override {
        _jacobianOplusXi << line_coeffs_[0],
                            line_coeffs_[1],
                            -line_coeffs_[0] * r_ * std::sin(angle_ + theta_) + line_coeffs_[1] * r_ * std::cos(angle_ + theta_);
