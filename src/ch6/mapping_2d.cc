...
bool Mapping2D::ProcessScan(Scan2d::Ptr scan) {
    ...
    if (is_kf) {
        ...
        sad::Icp2d icp;
        double means_1, cov_1, means_2, cov_2;
        icp.DegradationTesting(last_keyframe_->pose_, last_keyframe_->scan_, means_1, cov_1);
        icp.DegradationTesting(SE2(), current_frame_->scan_, means_2, cov_2);
        std::cout << "前后帧方差之差: " << cov_1 - cov_2 << std::endl;
