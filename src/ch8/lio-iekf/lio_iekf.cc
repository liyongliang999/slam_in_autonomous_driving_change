...
void LioIEKF::ProcessMeasurements(const MeasureGroup &meas) {
    ...
    // 对点云去畸变
    Undistort();
    std::cout << "undistort succeed" << std::endl;
    // 配准
    // AlignWithIcp();
    // AlignWithIcpSimple();
    Align();
...
void LioIEKF::AlignWithIcp() {
    ...
    /// the first scan
    if (flg_first_scan_) {
        icp_.ikdtree_ = std::make_shared<KD_TREE<PointType>>();
        icp_.ikdtree_->Build(current_scan_filter->points);
        flg_first_scan_ = false;
        // nav_state_prior_ = std::make_shared<PriorNavState>(last_nav_state_, Mat15d::Identity() * 1e4);
        return;
    }

    // 后续的scan，使用icp配合pose进行更新
    LOG(INFO) << "=== frame " << frame_num_;
    // pred_nav_state_ = ieskf_.GetNominalState();
    ...
    if (delta_pose.translation().norm() > 1.0 || delta_pose.so3().log().norm() > math::deg2rad(10.0)) {
        ...
        icp_.ikdtree_->Add_Points(current_scan_world->points, true);
        last_pose_ = current_pose;
    }
...
void LioIEKF::AlignWithIcpSimple() {
    FullCloudPtr scan_undistort_trans(new FullPointCloudType);
    ...
    icp_.SetSource(current_scan_filter);
    ieskf_.UpdateUsingCustomObserve([this](const SE3 &input_pose, Mat18d &HTVH, Vec18d &HTVr) {
        icp_.ComputeResidualAndJacobiansSimple(input_pose, HTVH, HTVr);
    });
