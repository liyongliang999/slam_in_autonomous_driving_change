...
本代码是在高博的源代码基础上，按照深蓝学院的课程要求，对各章的部分代码进行修改，以实现特定的功能．
源代码见地址：https://github.com/gaoxiang12/slam_in_autonomous_driving
...
- 第3章，将ESKF运动方程改为左乘模型，并给出代码实现
- 第4章，实现Odom触发的图优化；使用数值求导来验证解析求导结果的正确性
- 第5章，在三维体素中定义NEARBY14，实现14格最近邻查找；对比本节的最近邻算法和nanoflann算法，给出精度和时间指标
- 第6章，实现基于优化器的点到点ICP，点到线ICP；基于直线拟合，检测单个Scan退化
- 第7章，为三类手写ICP和手写NDT添加柯西鲁棒核函数；并将鲁棒核函数引入增量NDTLO和LooselyLIO；为LOAM like LO设计地面点云提取流程，并为地面点云使用点面ICP
- 第8章，将IEKF中的观测方程从增量NDT换成点面ICP
- 第9章，为本书手写NDT设计匹配度评估指标，并利用该指标提取回环；将离线点云地图改为NDT map并存储
- 第10章，实现基于NDT map的激光定位。根据车辆实时位姿，加载所需的NDT体素并完成定位。同样，要根据初始的RTK位置进行角度搜索
